...
export class JwtStrategy extends PassportStrategy(Strategy) {
  ...
  async validate(payload: any) {
    const usuario = await this.usuariosService.obtenerUsuario(payload.sub);
    ...
    const permisosExtra = usuario.permisosExtra.map(p => p.permiso);
    const permisosBloqueados = usuario.permisosBloqueados.map(p => p.permiso);
